...
const ElementContainer = (props: { name: string, category: string, price: string, categories: string[], _id: string, isPending: boolean, unitOfMea: string }) => {
    ...
    const delElement = async () => {
        try {
            const res = await deletePrice(props._id);
            if (res.error) alert(res.error);
        } catch (error) {
            console.log(error);
            alert('Hiba lépett fel a művelet során. Kérjük próbálja újra.')
        }
    }

    return (
        <li className="flex gap-4 mb-4 border-b-2 pb-2 items-center">
