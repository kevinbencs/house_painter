...
import { chooseTypeOfTextItem } from "./showPage";
...
const DynamicPagesForm = (props: {
    ...
    const  [titleParaghInput, setTitleParaghInput] = useState<string>('')
    ...
    return (
        <>
            <form action="" className='mb-20 mt-10' onSubmit={submitEvent}>
                ...
                <input type="text" name='title' className='focus-within:outline-none border-b-2 input-bordered block w-full mb-8 bg-transparent pl-2 dark:text-white' placeholder='Cím' value={titleParaghInput} onChange={(e) => setTitleParaghInput(e.target.value)} />
            ...
            <div >
                <div className="lg:flex gap-10 lg:justify-center bg-mist-900  text-white lg:content-center mt-20 mb-20 p-10">
                    <div className="hidden lg:block lg:max-w-[30%] ">
                        <HeadingImg id={coverImageId} />
                    </div>
                    
                    <section>
                        <h2 className=' text-5xl mb-10 font-bold leading-normal'>{titleInput}</h2>
                        <p>{titleParaghInput}</p>
                    </section>
                </div>
